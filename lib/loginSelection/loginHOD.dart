...
class LoginControllerHOD {
  static Future<void> loginHOD(BuildContext context, String selectedRole, TextEditingController emailController, TextEditingController passwordController) async {
   
    

     
      await Provider.of<AdminProvider>(context, listen: false)
          .findHod(emailController.text.trim(), passwordController.text.trim());

      log("message for the hod${Provider.of<AdminProvider>(context, listen: false).searchResult}");
      Timer(const Duration(seconds: 2), () {
        if (Provider.of<AdminProvider>(context, listen: false).searchResult == "HOD found!" && FirebaseAuth.instance.currentUser == null) {
          ...
          if (FirebaseAuth.instance.currentUser!.email == emailController.text.trim()) {
            ...
            Navigator.of(context).pushReplacement(MaterialPageRoute(builder: (context) => const Hodpage()));
